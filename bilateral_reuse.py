...
if __name__ == '__main__':
    ...
    #input_image = np.load('image.npz')['image'].astype(np.float32)[::2, ::2].copy()
    ...
    #input_image = np.load('image.npz')['image'].astype(np.float32)[0:5, 0:5].copy()
    ...
    local_size = (80, 4)  # 64 pixels per work group
    ...
    for halo_norm in range(1, 11):
        ...
        sigma = np.float32(4)
        # Set up a (N+2*halo x N+2*halo) local memory buffer.
        # 4 bytes for float.
        buf_height = 2
        ...
        prop_exec = program.bilateral_filtering_reuse(queue, global_size, local_size,
